...
```{r}
fit <- survfit(Surv(dataset$time, status==1) ~ 1, data = dataset)
...
min(dataset$age) #61
max(dataset$age) #97
fit.age <- survfit(Surv(time, status) ~ age, data=dataset)
...
log_rank_test <- survdiff(Surv(time, status) ~ cutpoint, data=dataset)
log_rank_test  #0.09
...
obs_NA <- which(is.na(dataset$MMSE))
